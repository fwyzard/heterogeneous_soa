...
CXXFLAGS := -std=c++17 -O2 -flto -g -Wall -fPIC -MMD -march=native -mtune=native -Isrc -isystem external/alpaka/include -isystem external/boost/include

ALPAKA_SERIAL_CXXFLAGS := -DALPAKA_ACC_CPU_B_SEQ_T_SEQ_ENABLED -DALPAKA_HOST_ONLY
ALPAKA_CUDA_CXXFLAGS   := -DALPAKA_ACC_GPU_CUDA_ENABLED -DALPAKA_HOST_ONLY -isystem /usr/local/cuda/include
ALPAKA_HIP_CXXFLAGS    := -DALPAKA_ACC_GPU_HIP_ENABLED -DALPAKA_HOST_ONLY -isystem /opt/rocm/include -isystem /opt/rocm/include/hip -isystem /opt/rocm/include/hiprand -D__HIP_PLATFORM_AMD__ -D__HIP_PLATFORM_HCC__
